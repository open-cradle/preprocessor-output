...
} namespace cradle { struct tasklet_machine_overview { std::string machine_name;  std::vector<tasklet_overview > tasklets;  tasklet_machine_overview() {} tasklet_machine_overview(std::string const& machine_name, std::vector<tasklet_overview > const& tasklets) : machine_name(machine_name), tasklets(tasklets) {} };  inline tasklet_machine_overview make_tasklet_machine_overview(  std::string machine_name, std::vector<tasklet_overview > tasklets ) { return tasklet_machine_overview(  std::move(machine_name), std::move(tasklets) ); }template<> struct definitive_type_info_query<tasklet_machine_overview> {     static void     get(cradle::api_type_info*); }; template<> struct type_info_query<tasklet_machine_overview> {     static void     get(cradle::api_type_info*); }; template<> struct structure_field_type_info_adder<tasklet_machine_overview> {     static void     add(std::map<std::string, cradle::api_structure_field_info>*); }; bool operator==(tasklet_machine_overview const& a, tasklet_machine_overview const& b); bool operator!=(tasklet_machine_overview const& a, tasklet_machine_overview const& b); bool operator<(tasklet_machine_overview const& a, tasklet_machine_overview const& b); void swap(tasklet_machine_overview& a, tasklet_machine_overview& b); size_t deep_sizeof(tasklet_machine_overview const& x); void write_fields_to_record(cradle::dynamic_map& record, tasklet_machine_overview const& x); void to_dynamic(cradle::dynamic* v, tasklet_machine_overview const& x); void read_fields_from_record(tasklet_machine_overview& x, cradle::dynamic_map const& record); void from_dynamic(tasklet_machine_overview* x, cradle::dynamic const& v); std::ostream& operator<<(std::ostream& s, tasklet_machine_overview const& x);std::ostream& operator<<(std::ostream& s, tasklet_machine_overview const& x);size_t hash_value(tasklet_machine_overview const& x);} namespace cradle { 

#line 354 "/home/runner/work/cradle/cradle/src/cradle/websocket/messages.hpp"
} namespace cradle { struct introspection_status_response { integer now;  std::vector<tasklet_machine_overview > machines;  introspection_status_response() {} introspection_status_response(integer const& now, std::vector<tasklet_machine_overview > const& machines) : now(now), machines(machines) {} };  inline introspection_status_response make_introspection_status_response(  integer now, std::vector<tasklet_machine_overview > machines ) { return introspection_status_response(  std::move(now), std::move(machines) ); }template<> struct definitive_type_info_query<introspection_status_response> {     static void     get(cradle::api_type_info*); }; template<> struct type_info_query<introspection_status_response> {     static void     get(cradle::api_type_info*); }; template<> struct structure_field_type_info_adder<introspection_status_response> {     static void     add(std::map<std::string, cradle::api_structure_field_info>*); }; bool operator==(introspection_status_response const& a, introspection_status_response const& b); bool operator!=(introspection_status_response const& a, introspection_status_response const& b); bool operator<(introspection_status_response const& a, introspection_status_response const& b); void swap(introspection_status_response& a, introspection_status_response& b); size_t deep_sizeof(introspection_status_response const& x); void write_fields_to_record(cradle::dynamic_map& record, introspection_status_response const& x); void to_dynamic(cradle::dynamic* v, introspection_status_response const& x); void read_fields_from_record(introspection_status_response& x, cradle::dynamic_map const& record); void from_dynamic(introspection_status_response* x, cradle::dynamic const& v); std::ostream& operator<<(std::ostream& s, introspection_status_response const& x);std::ostream& operator<<(std::ostream& s, introspection_status_response const& x);size_t hash_value(introspection_status_response const& x);} namespace cradle { 
...
#line 396 "/home/runner/work/cradle/cradle/src/cradle/websocket/messages.hpp"
} // namespace cradle
